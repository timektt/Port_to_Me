...
const NodeMobileMenu = ({ onClose, theme, setTheme }) => {
  ...
  return (
    <div className={`fixed top-0 left-0 w-64 h-full p-4 z-50 shadow-lg transition-all duration-300 
      ...
      {/* ✅ รายการบทเรียน Node.js */}
      <ul className="mt-3 space-y-3">
        <li>
          <button 
            onClick={() => { navigate("/courses/nodejs/101"); onClose(); }} 
            className="w-full flex justify-between items-center text-left hover:text-gray-300 transition"
          >
            101: Introduction to Node.js <FaChevronRight />
          </button>
        </li>
        <li>
          <button 
            onClick={() => { navigate("/courses/nodejs/201"); onClose(); }} 
            className="w-full flex justify-between items-center text-left hover:text-gray-300 transition"
          >
            201: Event Loop & Async <FaChevronRight />
          </button>
        </li>
        <li>
          <button 
            onClick={() => { navigate("/courses/nodejs/202"); onClose(); }} 
            className="w-full flex justify-between items-center text-left hover:text-gray-300 transition"
          >
            202: Building REST API <FaChevronRight />
          </button>
        </li>
      </ul>
    </div>
